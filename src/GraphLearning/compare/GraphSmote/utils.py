...
def get_parser():
    ...
    parser.add_argument('--batch_nums', type=int, default=600, help='number of batches per epoch')
    parser.add_argument('--batch_size', type=int, default=100, help='number of batches per epoch')

    parser.add_argument('--imbalance', action='store_true', default=False)
    parser.add_argument('--setting', type=str, default='newG_cls',
    ...
    parser.add_argument('--load', type=str, default='newG_cls_100_False_0.5_graph')
...
def split_arti(labels, ll, rr):
    ...
    # 5700   1   5699
    # 8015   5700 11415
    for i in range(0, 5699):  # train
        train_idx.append(i)
    for i in range(5699, 8014): # val
        val_idx.append(i)
    for i in range(ll, rr):    # test
        test_idx.append(i)
...
def recon_upsample(embed, labels, idx_train, adj=None, portion=1.0, im_class_num=1):
    ...
    adj_new = None
    # print(embed)
    # num_zeros = torch.sum(torch.eq(labels, 0))
    # num_ones = torch.sum(torch.eq(labels, 1))
    # print("Number of zeros: ", num_zeros.item())
    # print("Number of ones: ", num_ones.item())
    # print(c_largest, avg_number)
    for i in range(im_class_num):
        chosen = idx_train[(labels == (c_largest - i))[idx_train]]
        chosen1 = idx_train[(labels == (c_largest - 1))[idx_train]]
        num = int(chosen.shape[0] * portion)
        #print(num, chosen.shape[0])
        if portion == 0:
            #c_portion = int(avg_number / chosen.shape[0])
            c_portion = int((chosen1.shape[0] - chosen.shape[0]) / chosen.shape[0])
            #print(c_portion)
            num = chosen.shape[0]
        else:
            c_portion = 1
    ...
    num_zeros = torch.sum(torch.eq(labels[idx_train], 0))
    num_ones = torch.sum(torch.eq(labels[idx_train], 1))
    # print("Number of zeros: ", num_zeros.item())
    # print("Number of ones: ", num_ones.item())
    # print(len(idx_train))
    if adj is not None:
